...
public class Controller implements Serializable {
    ...
    public String  displayLibraryItems(int library, int mask){
        ...
        Library lib = getLib(library);
        if ((mask & 1) == 1){
            text += lib.displayItems("Book");
        }
        if ((mask & 2) == 2){
            text += lib.displayItems("CD");
        }
        if ((mask & 4) == 4){
            text += lib.displayItems("DVD");
        }
        if ((mask & 8) == 8){
            text += lib.displayItems("Magazine");
        }
        return text;
    }
    ...
    //
    public String displayCheckedOutItems(int libraryCardNum){
        String text = "";
            ArrayList<String> items = memberList.getMember(libraryCardNum).getCheckedOutItems();
            Item item;
            text += "Checked out items of member #: " + libraryCardNum + "\n";
            for (String element : items) {
                item = main.getItem(element);
                if (item != null) {
                    text += "Id = " + item.getId() + " " + item.getType() + " " + " Name = " + item.getName();
                    text+= " Due Date = "
                            + (item.getDateDue().get(Calendar.MONTH) + 1)
                            + "/" + item.getDateDue().get(Calendar.DAY_OF_MONTH)
                            + "/" + item.getDateDue().get(Calendar.YEAR) + "\n";
    ...
    //
    public boolean checkLibraryCardNumber(int cardNumber){
